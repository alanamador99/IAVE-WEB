...
/* Este componente react, servirá para renderizar un modal que permita editar/corregir información de un cruce seleccionado desde cualquiera de las ventanas */
const ModalHandleEditarCruce = ({ isOpen, onClose, onConfirm, cruceSeleccionado }) => {
  const cancelButtonRef = useRef(null);
  const [nombreCaseta, setNombreCaseta] = useState(cruceSeleccionado?.Caseta || '');
  const [nombreOperador, setNombreOperador] = useState(cruceSeleccionado?.No_Economico || '');
  const [ot, setOt] = useState(cruceSeleccionado?.id_orden || '');
  const [dictamen, setDictamen] = useState(cruceSeleccionado?.Dictamen || '');
  const [importe, setImporte] = useState(cruceSeleccionado?.Importe || '');
  // Para el campo de fecha, se necesita formatear la fecha del cruce seleccionado al formato aceptado por el input datetime-local
  const [fechaCruce, setFechaCruce] = useState(formatearFechaParaInput(cruceSeleccionado?.Fecha) || '');
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Escape' && isOpen) {
        onClose();
        ...
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content shadow-lg">
            ...
            <div className="modal-header bg-gradient-primary">
              <h5 className="modal-title text-white">
                <i className="fas fa-user-tie mr-2"></i>
                Editando cruce con ID: <small>{cruceSeleccionado?.ID}</small>
              </h5>
              <button
                type="button"
                className="close text-white"
            ...
            {/* Body */}
            <div className="modal-body">
              {cruceSeleccionado?.No_Economico && <div className="form-group">
                <label className="font-weight-bold text-gray-800">
                  Edita la información del cruce seleccionado. <span className="text-danger">*</span>
                </label>
                <input type="text" name="txtNombreCaseta" className="form-control form-control-sm border-left-info" placeholder='Nombre de caseta' onChange={(e) => setNombreCaseta(e.target.value)} value={nombreCaseta} id='txtNombreCaseta' />
                <input type="text" name="txtNombreOperador" className="form-control form-control-sm border-left-info" placeholder='Nombre de operador' onChange={(e) => setNombreOperador(e.target.value)} value={nombreOperador} id='txtNombreOperador' />
                <input type="text" name="txtOT" className="form-control form-control-sm border-left-info" placeholder='OT' onChange={(e) => setOt(e.target.value)} value={ot} id='txtOT' />
                <select name="selectDictamen" className="form-control form-control-sm border-left-info" placeholder='Dictamen' onChange={(e) => setDictamen(e.target.value)} value={dictamen} id='selectDictamen'>
                  <option value="">-- Selecciona un dictamen --</option>
                  <option value="Correcto">Confirmado</option>
                  <option value="Incorrecto">Abuso</option>
                  <option value="Dudoso">Aclaracion</option>
                  <option value="Sesgo">Sesgo</option>
                </select>

                <NumericFormat
                  name="txtImporte"
                  className="form-control form-control-sm border-left-info"
                  placeholder='Peaje 2 Ejes'
                  onValueChange={(values) => {
                    setImporte(values.floatValue || '');
                  }}
                  value={importe}
                  id='txtImporte'
                  thousandSeparator=","
                  ...
                  allowNegative={false}
                />
                <input type="datetime-local" name="txtFechaCruce" className="form-control form-control-sm border-left-info" placeholder='Fecha de cruce' onChange={(e) => setFechaCruce(e.target.value)} value={fechaCruce} id='txtFechaCruce' />
              </div>}
            </div>
            <div className="modal-footer">
              ...
              <button
                ...
                onClick={() => onConfirm({ DatosAGuardar: { caseta: nombreCaseta, No_Economico: nombreOperador, id_orden: ot, dictamen: dictamen, importe: importe, fechaCruce: fechaCruce } })}
              >
                Guardar cambios
              </button>
            </div>
