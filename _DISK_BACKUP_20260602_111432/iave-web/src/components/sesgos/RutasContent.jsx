...
const RutasContent = ({ onRutaSeleccionada, onOrigenDestino }) => {
    ...
    useEffect(() => {
        try {
            axios.get(`${API_URL}/api/sesgos/rutas`)
            .then(res => {
                setRutas(res.data);
    ...
    return (
        <div className="wrapper card shadow">
            <div className="card-header py-3 d-flex flex-column flex-md-row align-items-md-center justify-content-between">
                ...
                <h6 className="m-0 font-weight-bold text-primary ml-3" style={{ flex: 'auto', }}>🚛 ⚠️ Rutas con sesgos detectados- <i>IAVE-WEB</i> - ({formatearEnteros(filtered.length)}) ⚠️</h6>
            </div>
