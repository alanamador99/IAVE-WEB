...
export const getPrincipalesRutasConDiferencia = async (req, res) => {
    ...
    try {
        ...
        let query = `
        SELECT distinct TRN.Id_Ruta,  OT.ID_orden, TRN.id_Tipo_ruta, ot.ID_matricula, PO.Poblacion AS Origen, PD.Poblacion AS Destino, COUNT(*) AS RecuentoCruces, SUM(CR.Importe) AS ImporteCobrado, SUM(CR.ImporteOficial) AS ImporteOficial, (SUM(CR.ImporteOficial) - SUM(CR.Importe)) AS diferencia
        FROM cruces CR
            INNER JOIN Orden_traslados OT
            ON CR.id_orden = OT.ID_orden
        ...
        GROUP BY TRN.Id_Ruta, PO.Poblacion, PD.Poblacion, OT.ID_orden, TRN.id_Tipo_ruta, ot.ID_matricula
