...
export const getConciliacion = async (req, res) => {
  try {
    ...
    const { recordset: cruces } = await pool
      .request()
      .query(
        `SELECT ID, No_Economico, Fecha, Caseta, Importe FROM cruces WHERE Fecha BETWEEN DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE())-1, 1) AND GETDATE();`
      );

    console.log("⏺️  # de cruces leídos:", cruces.length);
    console.dir(cruces.slice(0, 5), { depth: 1 });
    // 2) Traer todo orden_status (ya incluye fk_orden y matrícula)
    const { recordset: otStatus } = await pool
      .request()
      .query(
        `SELECT fk_orden, fk_matricula, iniciada, finalizada FROM orden_status WHERE iniciada BETWEEN DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE())-1, 1) AND GETDATE();`
      );
    console.log("⏺️  # de status leídos:", otStatus.length);
    console.dir(otStatus.slice(0, 5), { depth: 1 });
    // 3) Procesar conciliación en JS
    const resultado = cruces.map((c) => {
      // Extraer matrícula (antes del primer espacio o todo el string)
      const matricula = c.No_Economico?.includes(" ")
        ? c.No_Economico.split(" ")[0]
        : c.No_Economico;

      // Filtrar status por esa matrícula
      const candidatos = otStatus.filter(
        (o) => String(o.fk_matricula) === String(matricula)
      );

      // Si no hay OT para esa matrícula
      if (candidatos.length === 0) {
        return {
          ...c,
          matricula,
          ID_orden: null,
          EstatusConciliacion: "Sin OT",
        };
      }

      // Tomar la primera OT (o podrías iterar si hay varias)
      const ot = candidatos[0];
      const { fk_orden: ID_orden, iniciada, finalizada } = ot;
      ...
      if (!iniciada || !finalizada) {
        return {
          ...
          matricula,
          ID_orden,
          EstatusConciliacion: "Sin rango",
        };
      }

      // Comparar fechas
      const fechaCruce = new Date(c.Fecha);
      const inicio = new Date(iniciada);
      const fin = new Date(finalizada);

      if (fechaCruce >= inicio && fechaCruce <= fin) {
        return {
          ...c,
          matricula,
          ID_orden,
      ...
      // OT existe pero el cruce está fuera del rango
      return {
        ...c,
        matricula,
        ID_orden,
        EstatusConciliacion: "Fuera de horario",
      };
    });
    ...
    res.status(500).send("Error al obtener conciliación");
  }
};
