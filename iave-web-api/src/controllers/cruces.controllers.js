...
const matriculasAdmins = [
  "1", "165", "166", "1436", "1224", "CarlosFTMP", "Frontier", "Highlander", "Versa", "Tacoma", "RAV4", "Camioneta", "Highlander", "2247"
];
...
export const updateCruce = async (req, res) => {
  const { DatosAGuardar } = req.body;
  try {
    const pool = await getConnection();
    console.log("Datos recibidos para actualización:", DatosAGuardar);
    ...
    const result =
      await pool.request()
        ...
        .input("OT", sql.VarChar, DatosAGuardar.id_orden)
        .input("nombreCaseta", sql.VarChar, DatosAGuardar.caseta)
        .input("nombreOperador", sql.VarChar, DatosAGuardar.No_Economico)
        .input("dictamen", sql.VarChar, DatosAGuardar.Dictamen)
        .input("importe", sql.Float, DatosAGuardar.Importe)
        .query(
          "UPDATE Cruces SET id_orden = @OT, Caseta = @nombreCaseta, No_Economico = @nombreOperador, Estatus = @dictamen, Importe = @importe WHERE ID = @id"
        );  
    console.log("Resultado de la actualización:", result);
    res.status(200).json({
      message: `Se colocó la ${DatosAGuardar.id_orden} actualizada correctamente sobre el ID ${req.params.id}`,
    });
  } catch (error) {
    console.error("Error actualizando estatus:", error);
    res.status(500).json({ error: "Error en el servidor" });
...
export const conciliarCrucesMasivo = async (req, res) => {
  ...
  try {
    const pool = await getConnection();
    const cruces = await pool.request().query(`
      SELECT ID, Tag, [No_Economico], Fecha, Caseta, Importe
      FROM Cruces
      WHERE ID IN (${"'"}${ids.join("','")}${"'"})
    `);

    let actualizados = 0;
    const totalCruces = cruces.recordset.length;
    let omitidos = {
      incompletos: 0,
      fechaInvalida: 0,
      duplicado: 0,
    ...
    sendProgressToClients({
      ...
      processed: 0,
      inserted: 0,
      actualizados: 0, // Nuevo campo para diferenciar update de insert
      skipped: 0,
      percentage: 0,
      message: 'Iniciando conciliación de cruces...'
    });

    // Iterar sobre cada cruce
    let i = 0;
    for (const cruce of cruces.recordset) {
      i++;
      ...
      if (i % 10 === 0 || i === 1 || i === totalCruces) {
        ...
        sendProgressToClients({
          ...
          skipped: omitidos.incompletos + omitidos.fechaInvalida + omitidos.duplicado,
          percentage: percentage,
          message: `Conciliando cruce ${i} de ${totalCruces}...`
        });
      }

      const {
        Tag,
        "No_Economico": NoEconomico, // Nota: En DB suele ser No_Economico, en CSV No.Economico
        Fecha, // Objeto Date desde DB
        Caseta,
        Importe,
        ID: IDCruce // El ID ya existe
      } = cruce;

      // Calcular fecha cruce (si viene de DB ya debería ser Date, pero por si acaso)
      // Nota: Si viene de DB SQL Server, Fecha ya es un objeto Date JS, no string.
      const fechaCruce = new Date(Fecha);
      let Estatus = null;

      // Lógica de Matrícula (Idéntica a importCruces)
      let matriculaInt = null;
      let NoEconomicoCruce = null;
      try {
        if ((matriculasAdmins).includes(NoEconomico?.trim().split(" ")[0])) {
        ...
        if (match.recordset.length > 0) {
          ...
          for (const caseta of recordsetTarifaOficial.recordset) {
            if (normalize(caseta.Nombre_IAVE) === normalize(Caseta)) {
              TarifaOficial = limpiarImporte(caseta.Importe);
              idCaseta = caseta.ID_Caseta;
              // Recalcular estatus basado en importe
              const importeLimpio = limpiarImporte(Importe);
              Estatus = ((importeLimpio - TarifaOficial) === 0 ? "Confirmado" : (importeLimpio - TarifaOficial) < 0 ? "Se cobró menos" : "Aclaración");
              EstatusSecundario = (Estatus === "Confirmado" ? "confirmado" : (Estatus === "Se cobró menos" ? "cobro_menor" : "pendiente_aclaracion"));
              break;
            } else {
              idCaseta = null;
          ...
          if (statusMatch.recordset.length > 0) {
            coincidencias = (statusMatch.recordset.length > coincidencias ? statusMatch.recordset.length : coincidencias);
            EstatusSecundario = (Estatus === "Administrativo" ? "Administrativo" : "");
            Estatus = (Estatus !== "Administrativo" ? "Pendiente" : 'Confirmado');
            for (const situacion of statusMatch.recordset) {
              const estatusPersonal = situacion.Descripcion;
              const descripcion = estatusPersonal?.toLowerCase() || "";
              const esAbuso = situacionesAbusivas.some((frase) =>
      ...
      // UPDATE UPDATE UPDATE: Actualizamos en lugar de insertar
      await pool.request()
        .input("ID", sql.NVarChar, IDCruce)
        .input("idCaseta", sql.NVarChar, idCaseta || '')
        .input("NoEco", sql.NVarChar, NoEconomicoCruce)
        .input("ImporteOficial", sql.Float, TarifaOficial || 0)
        .input("Estatus", sql.NVarChar, Estatus || "Vacío")
        .input("EstatusSecundario", sql.NVarChar, EstatusSecundario || "")
        .input("id_orden", sql.NVarChar, idOrdenAsignada)
        .query(`
    ...
    res.status(200).json({
      message: `Cruces conciliados correctamente. Total: ${totalCruces}, Actualizados: ${actualizados}`,
      details: { total: totalCruces, actualizados, omitidos }
    });

  } catch (error) {
    console.error("Error conciliando cruces:", error);
    // Enviar error a clientes SSE
    sendProgressToClients({
      type: 'error',
