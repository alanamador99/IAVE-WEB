...
export const actualizarEstatusMasivo = async (req, res) => {
  ...
  console.log("Actualizar estatus masivo", "ids", ids.join("','"));
  if (!Array.isArray(ids) || !nuevoEstatus) {
    return res.status(400).json({ error: "Datos inválidos" });
  }
  try {
    const pool = await getConnection();
    await pool.request().input("nuevoEstatus", sql.VarChar, nuevoEstatus)
      .query(`
        UPDATE cruces
        SET Estatus_Secundario = @nuevoEstatus
        WHERE ID IN (${ids.map(id => `'${id}'`).join(",")}) AND Estatus = 'Abuso'
      `);
