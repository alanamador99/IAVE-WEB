...
const Dashboard = () => {
  const [filters, setFilters] = useState({ fechaInicio: '', fechaFin: '', vOT: '', mat_OP: '', vCaseta: '', Estatus: '', rutaSeleccionada: '', vIdTipoRuta: '' });
  const { fechaInicio: vFechaInicio, fechaFin: vFechaFin, vOT, mat_OP: vmat_OP, vCaseta, Estatus: vEstatus, rutaSeleccionada, vIdTipoRuta } = filters;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'vIdTipoRuta') {
      // Si se selecciona una ruta, se actualiza el filtro de rutaSeleccionada con el Id_tipo_ruta correspondiente
      const rutaSeleccionada = data?.rutasActivas?.find(ruta => String(ruta.Id_tipo_ruta) === val);
      setFilters((prev) => ({
        ...prev,
        rutaSeleccionada: rutaSeleccionada ? String(rutaSeleccionada.Id_tipo_ruta) : '',
        [name]: val
      }));
    }
    else if (name === 'rutaSeleccionada') {
      // Si se selecciona una ruta, se actualiza el filtro de vIdTipoRuta con el Id_tipo_ruta correspondiente
      const rutaSeleccionada = data?.rutasActivas?.find(ruta => String(ruta.Id_tipo_ruta) === val);
      setFilters((prev) => ({
        ...prev,
  ...
  const resetFiltros = () => {
    setFilters({ fechaInicio: '', fechaFin: '', vOT: '', mat_OP: '', vCaseta: '', Estatus: '', rutaSeleccionada: '', vIdTipoRuta: '' });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const p1 = axios.post(`${API_URL}/api/dashboard/getDashboardData/`, {
          idOrden: vOT,
          Matricula: vmat_OP,
        ...
        const p2 = axios.get(`${API_URL}/api/dashboard/getRutasInfo/`).then(res => {
          setRutasData(res.data);
        }).catch(err => console.error('Error al cargar las rutas:', err));

        const p3 = axios.get(`${API_URL}/api/dashboard/getPrincipalesRutasConDiferencia`).then(res => {
          console.log('Rutas con mayor diferencia:', res.data);
        }).catch(err => console.error('Error al cargar las rutas con diferencia:', err));
  ...
  const ExportarDatosaExcel = (data, nombreArchivo) => {
    import('xlsx').then(XLSX => {
      ...
      const worksheetAclaraciones = XLSX.utils.json_to_sheet(data);
      const workbook = XLSX.utils.book_new();
      //Una hoja para los datos generales
      XLSX.utils.book_append_sheet(workbook, worksheet, 'Datos');
      //Otra hoja para las aclaraciones
      XLSX.utils.book_append_sheet(workbook, worksheetAclaraciones, 'Aclaraciones');

      //Una hoja adicional para la descripción de los filtros aplicados
      const filtrosAplicados = {
        'Fecha Inicio': vFechaInicio ? formatearFecha(vFechaInicio) : 'No aplicado',
        'Fecha Fin': vFechaFin ? formatearFecha(vFechaFin) : 'No aplicado',
        'OT': vOT || 'No aplicado',
      ...
      const filtrosSheet = XLSX.utils.json_to_sheet(Object.entries(filtrosAplicados).map(([key, value]) => ({ Filtro: key, Valor: value })));
      XLSX.utils.book_append_sheet(workbook, filtrosSheet, 'Filtros Aplicados');
  ...
  // Configuración para Importe por Ruta, que viene de la API y se actualiza dinámicamente según los filtros aplicados:
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            return context.label + ': ' + (context.parsed)?.toLocaleString() + ' (' + (context.parsed / totalCasetasCruzadas * 100).toFixed(2) + '%)';
          }
        }
      }
    },
  ...
  const ChartLegend = ({ data, prefijoDeUnidad = '' }) => (
        ...
        <div key={idx} className="col-6 mb-2">
          <div className="d-flex align-items-center">
            ...
            <span className="small text-gray-600">
              {label}: {(data.datasets[0].data[idx]) !== 0 ? prefijoDeUnidad : ''} {(data.datasets[0].data[idx]?.toLocaleString()) ? data.datasets[0].data[idx].toLocaleString() : '-'} ({((data.datasets[0].data[idx] / totalCasetasCruzadas) * 100).toFixed(2)}%)
            </span>
          </div>
        </div>
      ))}
      ...
      {(!loading)&& <div id="content">
        ...
        <div className="container-fluid">
          ...
          <div className="d-flex flex-wrap gap-2 mb-3">
            ...
            <div className='col-md-5 ml-2  p-2 input-group-text mb-0 ' style={{ maxWidth: 'min-content', }}>
            ...
            <div className="form-floating pr-2" style={{ maxWidth: '9rem', }}>
              <input type="number" step={1} min={0} name="vIdTipoRuta" placeholder="Id Tipo Ruta" className="form-control form-control-sm" onChange={handleChange} value={vIdTipoRuta} />
              <label className="form-label">Id Tipo Ruta</label>
            </div>
          ...
          <div className="row">
            ...
            <div className="col-xl-3 col-md-6 mb-4">
              <StatCard
                title="Aclaraciones procedentes"
                value={detalleAclaracionesAgrupadas?.length?.toLocaleString()}
                subtitle={`$${(detalleAclaracionesAgrupadas) && detalleAclaracionesAgrupadas?.reduce((acc, item) => acc + item.Importe, 0)?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} con el filtro aplicado`}
                icon={AlertCircle}
                color="#f6c23e"
                borderColor="#f6c23e"
              />
            ...
            <div className="col-xl-6 col-lg-6">
              <div className="card shadow mb-4">
                ...
                <div className="card-body">
                  ...
                  <ChartLegend data={presupuestoData} prefijoDeUnidad='$' />
                </div>
              </div>
            </div>
          </div>
